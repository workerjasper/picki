...
export default function LinkDetailClient({ id, initialLink }: Props) {
  const { user } = useAuth();
  const router = useRouter();
  const [link, setLink] = useState<LinkType | null>(initialLink);
  const [loading, setLoading] = useState(!initialLink);
  ...
  useEffect(() => {
    ...
    async function fetchUserState() {
      ...
      setLink((prev) => prev ? { ...prev, is_liked: !!likeData, is_saved: !!saveData } : prev);
    }
    fetchUserState();
  }, [user, id]);

  // initialLink 없을 때 직접 fetch (fallback)
  useEffect(() => {
